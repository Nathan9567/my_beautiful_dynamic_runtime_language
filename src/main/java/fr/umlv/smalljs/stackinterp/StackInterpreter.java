		...
		for (;;) {
			switch (instrs[pc++]) {
				...
				case Instructions.FUNCALL -> {
					...
					// decode qualifier
					var newFunction =
							(JSObject) decodeAnyValue(stack[baseArg + QUALIFIER_BASE_ARG_OFFSET], dict, heap);
					//{ // DEBUG
					...
					if (maybeCode == null) { // native call !
					  // decode receiver
					  var receiver =
								decodeAnyValue(stack[baseArg + RECEIVER_BASE_ARG_OFFSET],
										dict, heap);
          ...
          int activation = baseArg - 1 + code.slotCount();
				...
				case Instructions.RET -> {
					...
					sp = bp - 1;
					function =
              (JSObject) decodeDictObject(stack[activation + FUN_OFFSET], dict);
